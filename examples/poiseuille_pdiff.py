...
in_bc = api.Dirichlet(name='in_p', material=fluid)
in_bc.data[CUBA.VARIABLE] = CUBA.PRESSURE

face1z = api.Boundary(name='face1z', condition=[in_bc])
face1z.data[CUBA.DIRECTION] = (0, 0, -1)

out_bc = api.Dirichlet(name='out_p', material=fluid)
out_bc.data[CUBA.VARIABLE] = CUBA.PRESSURE

facez1 = api.Boundary(name='facez1', condition=[out_bc])
...
pgrad = fluid.data[CUBA.DENSITY]*kinem_pgrad  # gradient of pressure
pdiff = (L-1.0)*pgrad  # pressure difference between outlet and inlet
...
# Set the initial pressure and velocity field
for node in lat.iter():
    if node.data[CUBA.MATERIAL] == fluid:
        ijk = node.index
        p = node.data[CUBA.PRESSURE]
        node.data[CUBA.PRESSURE] = p + pin_off + ijk[2]*pgrad
