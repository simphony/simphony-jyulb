...
cdef class PyNodeSet:
    ...
    def get_n(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk):
        cdef int ijk_size
    ...
    def get_ijk(self, unsigned int n, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk):
        cdef int ijk_size
        ijk_size = ijk.shape[0]
...
cdef class PyLattice(PyNodeSet):
    ...
    def __cinit__(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] size,
                  cnp.ndarray[double, ndim=1, mode="c"] origin):
    ...
    def get_size(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] size):
        size[0] = self.size[0]
        size[1] = self.size[1]
        size[2] = self.size[2]
        
    def get_origin(self, cnp.ndarray[double, ndim=1, mode="c"] origin):
        origin[0] = self.origin[0]
        origin[1] = self.origin[1]
        origin[2] = self.origin[2]

    @classmethod
    def from_lattice(cls, PyLattice lat):
...
cdef class PyGeometry:
    ...
    def set_material_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk, unsigned char value):
        cdef unsigned int n
    ...
    def get_material_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk):
        cdef unsigned int n
        n = self._lattice.get_n(ijk)
        return self.thisptr.get_phase().get_val_n(n)
...
cdef class PyAbstractIsothermalData:
    ...
    def set_vel_n(self, unsigned int n, cnp.ndarray[double, ndim=1, mode="c"] vel):
        self.thisptr.velx().set_val_n(n, vel[0])
        self.thisptr.vely().set_val_n(n, vel[1])
        self.thisptr.velz().set_val_n(n, vel[2])

    def set_frc_n(self, unsigned int n, cnp.ndarray[double, ndim=1, mode="c"] frc):
        self.thisptr.frcx().set_val_n(n, frc[0])
        self.thisptr.frcy().set_val_n(n, frc[1])
        self.thisptr.frcz().set_val_n(n, frc[2])

    def set_den_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk, double den):
        self.set_den_n(self.get_n(ijk), den)

    def set_vel_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk,
                    cnp.ndarray[double, ndim=1, mode="c"] vel):
        self.set_vel_n(self.get_n(ijk), vel)

    def set_frc_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk,
                    cnp.ndarray[double, ndim=1, mode="c"] frc):
        self.set_frc_n(self.get_n(ijk), frc)

    def get_den_n(self, unsigned int n):
        return self.thisptr.den().get_val_n(n)

    def get_vel_n(self, unsigned int n, cnp.ndarray[double, ndim=1, mode="c"] vel):
        vel[0] = self.thisptr.velx().get_val_n(n)
        vel[1] = self.thisptr.vely().get_val_n(n)
        vel[2] = self.thisptr.velz().get_val_n(n)

    def get_frc_n(self, unsigned int n, cnp.ndarray[double, ndim=1, mode="c"] frc):
        frc[0] = self.thisptr.frcx().get_val_n(n)
        frc[1] = self.thisptr.frcy().get_val_n(n)
        frc[2] = self.thisptr.frcz().get_val_n(n)

    def get_den_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk):
        return self.get_den_n(self.get_n(ijk))

    def get_vel_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk, cnp.ndarray[double, ndim=1, mode="c"] vel):
        self.get_vel_n(self.get_n(ijk), vel)
        
    def get_frc_ijk(self, cnp.ndarray[cnp.uint32_t, ndim=1, mode="c"] ijk, cnp.ndarray[double, ndim=1, mode="c"] frc):
        self.get_frc_n(self.get_n(ijk), frc)
...
cdef class PyIsothermalData(PyAbstractIsothermalData):
    ...
    def __cinit__(self, PyNodeSet nodes, double iden, cnp.ndarray[double, ndim=1, mode="c"] ivel,
                  cnp.ndarray[double, ndim=1, mode="c"] ifrc):
